...
public class DeliverSmTest extends PduTest {
  ...
  public void splitMo() {
    final var part1 =
        "";

    final var pdu1 = deserialize(part1, GsmCharset.GSM);
    assertThat(pdu1.destination().address()).isEqualTo("1971");
    assertThat(pdu1.message()).startsWith("Test 4");
    assertThat(pdu1.numberOfParts()).isEqualTo(10);
    assertThat(pdu1.part()).isEqualTo(1);
    assertThat(pdu1.concatenatedRef()).isEqualTo("2F");

    final var part2 =
        "";

    final var pdu2 = deserialize(part2, GsmCharset.GSM);
    assertThat(pdu2.destination().address()).isEqualTo("1971");
    assertThat(pdu2.message()).startsWith("g, Harry");
    assertThat(pdu2.numberOfParts()).isEqualTo(10);
    assertThat(pdu2.part()).isEqualTo(2);
    assertThat(pdu2.concatenatedRef()).isEqualTo("2F");
  }
  ...
  public void testCarrierCode() {
    final var encoded =
        "";
  ...
  public void testTeliaDk() {
    ...
    final var encoded =
        "";
    final var pdu = deserialize(encoded, GsmCharset.GSM);

    assertThat(pdu.destination().address()).isEqualTo("1971");
    assertThat(pdu.sender().address()).isEqualTo("4527584373");
    assertThat(pdu.receiptedMsgId()).isEqualTo("CE872BADEFAD47AA96B5D009DC73B29C01");
    assertThat(pdu.state()).isEqualTo((byte) 2);
  }
